...
import { Building2, FileUp, Loader2, RefreshCw } from "lucide-react";
import { cn } from "@/lib/utils";

/**
 * AI Investment Firm deck review market (Phase 25A).
 * Uploads a real PDF deck to /api/vc-review-jobs, polls the safe job view,
 * and renders firm cards + the aggregate funding decision. All firms are
 * fictional game entities; this panel never sees deck text after upload.
 */
  ...
  async function submit() {
    ...
    try {
      ...
      if (!response.ok && response.status !== 202) {
        setError(data.error ?? "Deck review submission failed.");
      ...
      setError("Network error while uploading the deck.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="border border-violet-500/20 bg-violet-500/[0.04] p-5">
      ...
      <p className="mb-4 text-xs leading-relaxed text-white/45">
        Upload your real PDF pitch deck and fictional arena investment firms will review it like a funding
        market. Game-world feedback only — never a real offer, never shared publicly.
      </p>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className={cn(
                "border px-2 py-1 text-[11px] font-bold uppercase tracking-wider",
                autoSelect ? "border-violet-400/50 bg-violet-500/15 text-violet-200" : "border-white/10 text-white/40"
              )}
            >
              Auto-select firms
            </button>
              ...
              {firms.map((firm) => (
                ...
                >
                  ...
                  <p className="text-[11px] text-white/40">
                    {firm.sectorFocus.slice(0, 3).join(" · ")} · {firm.checkSizeRange}
                  </p>
                </button>
              ))}
            </div>
          ...
          <button
            type="button"
            onClick={submit}
            disabled={!file || submitting || (!autoSelect && selectedFirmIds.length === 0)}
          ...
          >
            ...
            Submit deck to the market
          </button>
        </div>
      )}

      {error && (
        <p className="mt-3 border border-rose-500/25 bg-rose-500/10 p-2 text-xs text-rose-300">{error}</p>
      )}
        ...
        <div className="mt-3 flex items-center gap-3 border border-white/10 bg-white/[0.02] p-3 text-xs text-white/60">
          ...
          <span>
            {job.status === "reviewing"
              ? "Arena firms are reviewing your deck… this can take a couple of minutes."
              : "Reading your deck…"}
          </span>
        </div>
      )}
          ...
          <div className={cn("border p-3", OVERALL_STYLES[job.aggregateReview.overallDecision])}>
            <div className="flex items-center justify-between gap-2">
              <p className="text-sm font-black uppercase tracking-wider">
                Market verdict: {job.aggregateReview.overallDecision}
              </p>
              <p className="text-xs font-bold">
                {job.aggregateReview.overallScore}/100 · funding likelihood {job.aggregateReview.fundingLikelihood}
              </p>
            </div>
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            {(job.firmReviews ?? []).map((review) => (
              <div key={review.firmId} className="border border-white/10 bg-white/[0.02] p-3">
                ...
                {review.checkSizeSuggestion && (
                  <p className="text-[11px] text-white/45">Check size view: {review.checkSizeSuggestion}</p>
                )}
                {review.mainConcerns.length > 0 && (
                  <p className="mt-1 text-[11px] text-amber-300/80">Concerns: {review.mainConcerns.slice(0, 2).join(" · ")}</p>
                )}
                {review.questionsForFounder.length > 0 && (
                  <p className="mt-1 text-[11px] text-white/40">Asks: {review.questionsForFounder.slice(0, 2).join(" · ")}</p>
                )}
              </div>
            ))}
          </div>
